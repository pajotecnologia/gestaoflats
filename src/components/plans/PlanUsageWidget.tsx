...
export default function PlanUsageWidget({
  ...
  const getTextColor = (pct: number) => {
    ...
    return "text-slate-700 dark:text-zinc-300 font-semibold";
  };

  if (compact) {
    return (
      <div className={`p-3 rounded-2xl bg-slate-50 dark:bg-zinc-900/80 border border-slate-200 dark:border-zinc-800 space-y-2.5 backdrop-blur-md ${className}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <Zap className="w-3.5 h-3.5 text-emerald-500" />
            <span className="text-[11px] font-extrabold text-slate-900 dark:text-zinc-100 uppercase tracking-wider">
              {usage.plan.name}
            </span>
          </div>
          <Link
        ...
        <div className="space-y-1">
          <div className="flex justify-between text-[10px]">
            <span className="text-slate-500 dark:text-zinc-400">Imóveis</span>
  ...
  return (
    <div className={`p-5 rounded-3xl bg-white dark:bg-zinc-900/80 border border-slate-200 dark:border-zinc-800 shadow-xl backdrop-blur-md space-y-4 ${className}`}>
      <div className="flex items-center justify-between border-b border-slate-100 dark:border-zinc-800/80 pb-3">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xs font-black uppercase tracking-wider text-slate-900 dark:text-zinc-100">
              {usage.plan.name}
            </span>
            {usage.isTrial && (
        ...
        <Link
          href={`/renovar?plano=${usage.nextPlanForUpgrade?.slug || "PROFISSIONAL"}`}
          className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-xs flex items-center gap-1.5 shadow-lg shadow-emerald-500/20 transition-all duration-200"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        {/* 1. Imóveis */}
        <div className="space-y-1.5 p-3 rounded-2xl bg-slate-50 dark:bg-zinc-950/60 border border-slate-100 dark:border-zinc-800/60">
