    ...
    <Shell>
      <div className="space-y-6">
        ...
        {entradaCompare && saidaCompare && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4"><div className="max-h-[88vh] w-full max-w-5xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl dark:bg-slate-900">
          ...
          <div className="mt-5 flex flex-wrap justify-end gap-2"><button disabled={saidaCompare.cobrancaDanosGerada || saidaCompare.valorDanos <= 0} onClick={async()=>{const res=await fetch("/api/vistorias",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:saidaCompare.id,acao:"COBRAR_DANOS"})});const data=await res.json();setMessage(res.ok?"Cobrança de danos gerada em Contas a Receber.":data.error||"Não foi possível gerar a cobrança.");if(res.ok){setSaidaCompare({...saidaCompare,cobrancaDanosGerada:true});load();}}} className="rounded-xl bg-amber-600 px-4 py-2 text-xs font-bold text-white disabled:opacity-40">{saidaCompare.cobrancaDanosGerada?"Cobrança já gerada":"Gerar cobrança dos danos"}</button><button onClick={()=>{setEntradaCompare(null);setSaidaCompare(null)}} className="rounded-xl border px-4 py-2 text-xs font-semibold dark:border-slate-700">Fechar</button></div>
        </div></div>}
